...
# Defaults
segment_fmt = ".SHELL/segmented-%s"
temp_directory = ".SHELL"
...
class TridentProcess(dag.GridProcess):
    def start(self):
        import subprocess
        def set_niceness():
            if hasattr(self, "nice") and self.nice:
                from os import nice
        ...
        retval = subprocess.call([self.cmd] + self.args.split(), preexec_fn=set_niceness, stdout=stdout_file, stderr=stderr_file)
...
""".format(dna_basename,self.args,mirna_basename, dna_basename))
    ...
    def get_dag_node(self):
        ...
        full_args = "%s %s" % (" ".join([F.full_path() for F in infiles]), self.args)
        retval = TridentProcess("trident",infiles,output,arguments = full_args,rsc_fpops_est = fpops_est,rsc_fpops_bound = fpops_est*5,rsc_memory_bound = self.rsc_memory_limit)
