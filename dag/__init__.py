...
Creates and manages a Directed Acyclic Graph of embarrassingly parallel
 work units.
"""
...
def enum2string(enum, val):
    """
    Looks up the given integer and provides the string representation
     within the enum. If there is not name for the provided integer,
     None is returned.
...
def string2enum(enum, string):
    """
    Translates an string name for a value in an enum to the integer value
     of the enum.
...
class File:
    ...
    @type max_nbytes: int
    @ivar dir: Path of the directory in which the file is located on disk.
     Null string indicates the file is in the same directory as the dag file.
    @type dir: str
    """
    def __init__(self, physical_name, logical_name=None, temporary_file=False,
                 max_nbytes=15000000):
    ...
    def full_path(self):
        """
        Returns a full path of the File, if the directory is provided.
...
class Process(object):
    ...
    def __str__(self):
        raise DagException("String function must be overloaded classes"
                           " that extend dag.Process")

    def start(self):
        raise DagException("start function must be overloaded classes"
                           " that extend dag.Process")
    ...
    def clean(self, root_dag):
        """
        Cleans a Process's temporary files and removes it from a DAG
         (if provided).
...
class GridProcess(Process):
    ...
    @type deadline: int
    """
    def __init__(self, cmd, input_files, output_files, arguments,
                 rsc_fpops_est=10 ** 10, rsc_fpops_bound=10 ** 11,
                 rsc_memory_bound=536870912, deadline=None):
        ...
        # Temporary files that will be removed when the process is removed
        # from the DAG
        self.temp_files = []
    ...
    def __str__(self):
        ...
        if self.workunit_template and self.workunit_template.physical_name:
            retval += "%s: %s\n" % ("Workunit Template",
                                    self.workunit_template.physical_name)
        if self.result_template and self.result_template.physical_name:
            retval += "%s: %s\n" % ("Result Template",
                                    self.result_template.physical_name)
        retval += "args: %s\n" % self.args
        retval += "Children:\n"
...
class Graph(dict):
    """
    Graph is a dict that maps process output files names to the processes
     that produce them.
    """
...
class DAG:
    """
    DAG is a directed acyclic graph. It stores a list of processes
     (nodes in the graph). The Graph class connects nodes
     by their input/output files.
    ...
    def get_process(self, wuname):
        """
        Finds a process based on its name. If the workunit name is not found,
         UUID is tried instead.
    ...
    def save(self, filename=None, backup_first=True):
        ...
        @type filename: str
        @param backup_first: Whether or not the DAG file should be backed up
         before overwritting. Default: True
    ...
    def incomplete_prereqs(self, proc):
        """
        Checks a process instance within a DAG to see if all
         of its prerequisites are met.
        ...
        for infile in proc.input_files:
            if infile in self.graph:
                for parent in self.graph[infile]:
